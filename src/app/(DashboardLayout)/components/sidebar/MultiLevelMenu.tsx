...
const ListLabel = styled(Paragraph)<{ compact?: number }>(({ theme, compact }) => ({
  fontWeight: 700,
  fontSize: '12px',
  marginTop: '20px',
  marginLeft: '15px',
...
export const StyledText = styled(Span)<{ compact?: number; active?: number }>(({ theme, compact, active }) => ({
  whiteSpace: 'nowrap',
  paddingLeft: '0.8rem',
  transition: 'all 0.15s ease',
  fontSize: '13px',
...
const MultiLevelMenu: React.FC<MultiLevelMenuProps> = ({ sidebarCompact = false }) => {
  ...
  const renderLevels = (data: NavItem[]) => {
    return data.map((item, index) => {
      ...
      if (item.children) {
        ...
        const validChildren =
          item.children
            ?.filter(child => typeof child.name === 'string' && typeof child.path === 'string')
            .map(child => ({
              ...child,
